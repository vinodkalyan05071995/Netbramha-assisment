var App = (() => {
  // src/js/sections/guage.js
  var CreditScoreComponent = class extends HTMLElement {
    constructor() {
      super();
      ...
      const settings = {
        ...
        plugins: [{
          ...
          afterDatasetsDraw(chart) {
            const lastIndex = 7;
            const highlightIndexes = [3, 4, 5, 7];
            const { ctx } = chart;
            chart.data.datasets[0].data.forEach((value, index) => {
              if (!highlightIndexes.includes(index)) return;
              const meta = chart.getDatasetMeta(0);
              const point = meta.data[index].x !== void 0 ? meta.data[index] : meta.data[index].getProps(["x", "y"], true);
              const x = point.x;
              const y = point.y;
              ctx.beginPath();
